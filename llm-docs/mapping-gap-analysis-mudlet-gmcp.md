...
|---|---|---|
| Room identity | Single permanent scalar id per room, sent every move (IRE `num`, Aardwolf `num`, MSDP `VNUM`, MMP `room id`); sentinel for unmappable rooms | ✅ crc-of-path int ids via `get_room_id()`; stable across reboots; `no_room_id` attr → 0 = unmappable (matches Aardwolf's `num:-1` convention) |
